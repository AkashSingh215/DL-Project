import torchvision
import os
import utilscopyfuture.DataTools as dt
...
class CelebDF(selfdataset):
    def __init__(self, folder_path="DataSet/train_test_images_cropped/Celeb-DF/"):
...
class DFFD(selfdataset):
    def __init__(self, folder_path="DataSet/train_test_images_cropped/FakeImgDatasets/"):
        super(selfdataset, self).__init__()
        self.folder_path = folder_path
...
# DFFD Group A
class DFFDA(selfdataset):
    def __init__(self, folder_path="DataSet/train_test_images_cropped/FakeImgDatasets/"):
        super(selfdataset, self).__init__()
        self.folder_path = folder_path
        self.R_dir = ["YouTube", "ffhq"]  # , "celeba_2w"
        self.F_dir = ["Deepfakes", "FaceSwap", "Face2Face"]
        self.trainpath = lambda path, file: os.path.join(self.folder_path, file, "train")
        self.validpath = lambda path, file: os.path.join(self.folder_path, file, "valid")
        self.testpath = lambda path, file: os.path.join(self.folder_path, file, "test")

# DFFD Group B   
class DFFDB(selfdataset):
    def __init__(self, folder_path="DataSet/train_test_images_cropped/FakeImgDatasets/"):
        super(selfdataset, self).__init__()
        self.folder_path = folder_path
        self.R_dir = ["YouTube", "ffhq"]  # , "celeba_2w"
        self.F_dir = ["stylegan_celeba", "stylegan_ffhq", "faceapp", "stargan", "pggan_v1", "pggan_v2"]
        self.trainpath = lambda path, file: os.path.join(self.folder_path, file, "train")
        self.validpath = lambda path, file: os.path.join(self.folder_path, file, "valid")
        self.testpath = lambda path, file: os.path.join(self.folder_path, file, "test")

# DFFD_Future      
class DFFD_FUTURE(selfdataset):
    def __init__(self, folder_path="DataSet/train_test_images_cropped/FakeImgDatasets/"):
        super(selfdataset, self).__init__()
        self.folder_path = folder_path
        self.R_dir = ["YouTube", "ffhq"]
        self.F_dir = ["FaceShifter", "NeuralTextures"]
        self.trainpath = lambda path, file: os.path.join(self.folder_path, file, "train")
        self.validpath = lambda path, file: os.path.join(self.folder_path, file, "valid")
        self.testpath = lambda path, file: os.path.join(self.folder_path, file, "test")